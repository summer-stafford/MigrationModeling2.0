...
\title{Comparing $C^r$ Values with Network Perturbation Results - Focal Species: Pintail with Harvest}
\date{}       
...
load("DataFiles/PERTdataHarvest_new_nopositive.RData") # Load R data from the file
...
\item We assume a harvest rate (or kill rate) of k=0.08 "moderate harvest rate" from Mattsson et al.
\end{itemize}
...
% 
The baseline simulation including harvesting with a kill rate $k=.08$ has results that compare to {\it{A modeling framework for integrated harvest and habitat management of North American waterfowl: Case-study of northern pintail metapopulation dynamics}} by Mattsson et. al. For the case of harvesting  we find that the long term carrying capacity for each node during each season, including all classes, is
...
For the following plots we only perturb Breeding and Winter seasons, keeping survival rates during the flyover at 1.
...
\caption{Perturbation results: Range of K values after perturbations to each node, X-axis represents baseline population weighted $C^r$ value for each node}\label{fig:pintail_barcr_averageCR}
\end{center}
\end{figure}
