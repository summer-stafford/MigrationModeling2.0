...
load("DataFiles/PERTdataNOHarvest_new_nopositive_new.RData") # Load R data from the file
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=\textwidth, height=.8\textwidth]{RGraphics-pintail_barcr_averageCR}
\caption{Perturbation results: Range of K values after perturbations to each node, X-axis represents {\bf{baseline population weighted $C^r$ values}} for each node}\label{fig:pintail_barcr_averageCR}
\end{center}
\end{figure}
...
\caption{Perturbation results: Range of K values after perturbations to each node, X-axis represents baseline average $W^r$ value for each node}\label{fig:pintail_barcr_WR}
\end{center}
\end{figure}
...
<<label=pintail_barcr_CRseasons, fig=TRUE, include=FALSE, echo=FALSE, eps=FALSE, pdf=TRUE>>=
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=.8\textwidth, height=.8\textwidth]{RGraphics-pintail_barcr_CRseasons}
\caption{Perturbation results: Range of K values after perturbations to each node, X-axis represents baseline average CR value for each node, here we have split up the seasons - no average}\label{fig:pintail_barcr_WR}
\end{center}
\end{figure}
