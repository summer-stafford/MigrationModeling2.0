...
load("DataFiles/PERTdataElk.RData") # Load R data from the file
...
node_color <- c("red", "green", "blue") #YS, NBM, CYR
trans_color <- c("purple", "magenta") #YS -> CYR, NBM -> YS
@
...
<<label=elk_barcr_averageCR, fig=TRUE, include=FALSE, echo=FALSE, eps=FALSE, pdf=TRUE>>=
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=\textwidth, height=.8\textwidth]{RGraphics-elk_barcr_averageCR}
...
\begin{tabular}{|c|c|c|}
\hline
{\color{red}Yellowstone} & $\Box$ & breeding \\
\hline
{\color{green}Non-breeding Migratory} & $\bigcirc$ & breeding \\
\hline
{\color{blue}Cody Year Round} & $\triangle$ & non-breeding \\
\hline
\end{tabular}
...
<<label=elk_barcr_WR, fig=TRUE, include=FALSE, echo=FALSE, eps=FALSE, pdf=TRUE>>=
...
<<label=elk_barcr_CRseasons, fig=TRUE, include=FALSE, echo=FALSE, eps=FALSE, pdf=TRUE>>=
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=.8\textwidth, height=.8\textwidth]{RGraphics-elk_barcr_CRseasons}
\caption{Perturbation results: Range of K values after perturbations to each node, X-axis represents baseline average CR value for each node, here we have split up the seasons - no average}\label{fig:elk_barcr_WR}
\end{center}
\end{figure}
