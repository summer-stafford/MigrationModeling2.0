...
node_color <- c("red", "orange", "blue") # BO NBO YR
@
...
<<label=elkbaseline, fig=TRUE, include=FALSE, echo=FALSE, eps=FALSE, pdf=TRUE, fig.path=paste(SIMNAME)>>=
...
plot(0, 0, 
     ...
     main="Range of Network Carying Capacities (K) vs Baseline CR",  
     xlab="Node Baseline Population Weighted Average $C^r$", 
     ylab="Network K")
symb <- 0
...
plot(0, 0, 
     ...
     ylim = c(min(PLOTMINK),max(PLOTMAXK)), 
     main="Range of Network Carying Capacities (K) vs Baseline $W^r$",  
     xlab="Node Baseline Average $W^r$", 
     ylab="Network K")
symb <- 0
...
<<label=elk_barcr_DS, fig=TRUE, include=FALSE, echo=FALSE, eps=FALSE, pdf=TRUE>>=
...
plot(0, 0, 
     ...
     ylim = c(min(PLOTMINK),max(PLOTMAXK)), 
     main="Range of Network Carying Capacities (K) vs Baseline D_s",  
     xlab="Node Baseline $D_s$", 
     ylab="Network K")
symb <- 0
...
<<label=elk_barcr_KR, fig=TRUE, include=FALSE, echo=FALSE, eps=FALSE, pdf=TRUE>>=
