...
for (p in 1:length(PERT)){
  ...
  for (cl in 1:NUMNET){
    ...
    for (SN in 1:seasons){  # perturb all seasons of breeding and winter !!!!!!!!!!! THIS MUST BE CHANGED FOR EACH SPECIES !!!!!!!!!!!!!!
        alpha[[cl]][[SN]]$s0[[num]] <- BASE_alpha[[cl]][[SN]]$s0[[num]]*PERT[p]
        # CANT MAKE SURVIVAL RATES GREATER THAN 1
        if(alpha[[cl]][[SN]]$s0[[num]]>1){ 
          print(paste("Survival rate for PERT: ", PERT[p], "on Node", num, "season", SN, "is",alpha[[cl]][[SN]]$s0[[num]], ". Resetting to 1", sep=" " ))
          alpha[[cl]][[SN]]$s0[[num]] <- 1 
          }
    }
  }
