...
NETNAME <- c("seeds","plants") # Give a distinct name for each class as used in input files
# Order is important here we would index [[1]] = class 1 and [[2]] = class 2 in alpha and beta.

SAVENAME <- "CodePlants_NewEqn" # Specifies which folder contains species specific data

PERTNAME <- "PERTplants" # Specifies the name of the .r perturbation file

PATH_PERTURB <- "Perturb_Document_Code_Data" # Specifies which folder contains perturbation data

PERTSAVE <- "PERTdataPlant" # Specifies the name of the .RData file where the perturbation data will be saved

RNW <- "PlantAnalysisReviewJune2018" # Specifies .rnw file to use when producing PDF
...
num_nodes <- 8 # Number of nodes in the network
